...
video_path = r'G:\Ai Projects\Day 4\Vehicle Counting On different Lanes\vedio.mp4'
cap = cv2.VideoCapture(video_path)
model = YOLO('G:\Ai Projects\Day 4\Vehicle Counting On different Lanes\yolo11n.pt')
...
while True:
    ...
    for result in track_results:
        ...
        # Displaying the vehicle counts with red text and white border
        cvzone.putTextRect(frame, 
                   f'LANE A Vehicles = {len(zoneAcounter)}', 
                   [800, 99], 
                   thickness=3, 
                   scale=2.3, 
                   border=2, 
                   colorR=(0, 0, 255),    # Red text
                   colorB=(255, 255, 255) # White border
                  )

        cvzone.putTextRect(frame, 
                   f'LANE B Vehicles = {len(zoneBcounter)}', 
                   [800, 140], 
                   thickness=4, 
                   scale=2.3, 
                   border=2, 
        ...
        cvzone.putTextRect(frame, 
                   f'LANE C Vehicles = {len(zoneCcounter)}', 
                   [800, 180], 
                   thickness=4, 
                   scale=2.3, 
                   border=2, 
    ...
    # cv2.imshow('Resized Video Feed', frame_resized)
